...
;;; org-caldav.el --- Sync org files with external calendar through CalDA
...
(aio-defun org-caldav-check-dav (url)
  "Check if URL accepts DAV requests.
Report an error with further details if that is not the case."
  (let* ((response (aio-await (org-caldav-request url "OPTIONS")))
         (status (request-response-status-code response)))
    ;; can this happen?
    (when (not response)
      (error "Retrieving URL %s failed." url))
    (when (not (<= 200 status 299))
      (error "Error while checking for OPTIONS at URL %s: %s" url status))
    (let ((davheader (request-response-header response "dav")))
      (when (not davheader)
...
(defun aio-request (url &rest args)
...
(aio-defun org-caldav-request (url &optional
                                   request-method
                                   request-data
                                   extra-headers)
...
(aio-defun org-caldav-request-raw (url &rest args)
  (let ((response (aio-await (apply #'org-caldav-request url args))))
    (with-current-buffer (generate-new-buffer "*response*")
...
(aio-defun org-caldav-url-dav-get-properties (url property)
...
Also sets `org-caldav-empty-calendar' if calendar is empty."
  ...
  (aio-await (org-caldav-check-dav (org-caldav-events-url)))
  (let* ((output (aio-await (org-caldav-url-dav-get-properties
...
(aio-defun org-caldav-get-event-etag-list ()
...
(aio-defun org-caldav-get-event (uid &optional with-headers)
...
(aio-defun org-caldav-put-event (buffer)
  ...
  (let ((event (with-current-buffer buffer (buffer-string))))
    (with-temp-buffer
      ...
      (let* ((uid (org-caldav-get-uid))
        ...
        (aio-await (org-caldav-save-resource
                    (concat (org-caldav-events-url) uid org-caldav-uuid-extension)
                    (encode-coding-string (buffer-string) 'utf-8)))))))

(aio-defun org-caldav-url-dav-delete-file (url)
  "Delete URL.
Will switch to OAuth2 if necessary."
  (aio-await (org-caldav-request url "DELETE")))

(aio-defun org-caldav-delete-event (uid)
...
(aio-defun org-caldav-delete-everything (prefix)
...
Are you really sure? ")))
      (let ((events (aio-await (org-caldav-get-event-etag-list)))
            (counter 0)
            (url-show-status nil))
        (dolist (cur events)
          (setq counter (1+ counter))
          (message "Deleting event %d of %d" counter (length events))
          (aio-await (org-caldav-delete-event (car cur))))
        (setq org-caldav-empty-calendar t))
      (when (file-exists-p
...
(aio-defun org-caldav-update-eventdb-from-cal ()
...
(aio-defun org-caldav-sync-calendar (&optional calendar resume)
    ...
    (cl-progv (mapcar 'org-caldav-var-for-key calkeys) calvalues
      ...
      (let ((numretry 0)
        ...
        (while (null success)
          (condition-case err
              (progn
                (aio-await (org-caldav-check-connection))
                (setq success t))
            (error
      ...
      (unless resume
        ...
        (aio-await (org-caldav-update-eventdb-from-cal)))
      (aio-await (org-caldav-update-events-in-cal org-caldav-ics-buffer))
      (aio-await (org-caldav-update-events-in-org))
...
ICSBUF is the buffer containing the exported iCalendar file."
  ...
  (with-current-buffer icsbuf
    ...
    (let ((events (append (org-caldav-filter-events 'new-in-org)
          ...
          (counter 0)
          (url-show-status nil)
          (event-etag (aio-await (org-caldav-get-event-etag-list)))
          uid)
      ...
      (dolist (cur events)
        ...
        (if (eq (org-caldav-event-etag cur) 'put)
          ...
          (aio-await (org-caldav-set-sequence-number cur event-etag))
          (message "Putting event %d of %d" counter (length events))
          (if (aio-await (org-caldav-put-event icsbuf))
            ...
            (push (list org-caldav-calendar-id (car cur)
                  ...
                  org-caldav-sync-result))))
      ;; Get Etags
      (setq event-etag (aio-await (org-caldav-get-event-etag-list)))
      (dolist (cur events)
...
(aio-defun org-caldav-set-sequence-number (event event-etag)
  ...
  (save-excursion
    (let ((seq (org-caldav-event-sequence event))
      ...
      (unless (or seq
        ...
        (setq retrieve (aio-await (org-caldav-get-event (car event))))
        (when (null retrieve)
          ...
          (setq seq 1))
        (unless seq
          (with-current-buffer (aio-await (org-caldav-get-event (car event)))
            (goto-char (point-min))
...
(aio-defun org-caldav-update-events-in-org ()
  ...
  (let ((events (append (org-caldav-filter-events 'new-in-cal)
    ...
    (dolist (cur events)
      (catch 'next
        ...
        (with-current-buffer (aio-await (org-caldav-get-event uid))
          ;; Get sequence number
          (goto-char (point-min))
          (save-excursion
            (when (re-search-forward "^SEQUENCE:\\s-*\\([0-9]+\\)" nil t)
...
(defun org-caldav-get-calendar-summary-from-uid (uid)
  ...
  (let ((buf (aio-await (org-caldav-get-event uid)))
        (heading ""))
    (when buf
      (with-current-buffer buf
        (goto-char (point-min))
...
;; in case of an error.
(aio-defun org-caldav-save-resource (url obj)
